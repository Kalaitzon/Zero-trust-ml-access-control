...
"""
=============================================================================
 TASK 6 (γραφήματα + group breakdown) + TASK 7 (drift) + ΠΡΟΑΙΡΕΤΙΚΟ (fairness)

TASK 6 (Αξιολόγηση & επεξηγησιμότητα - συνέχεια):
  Παράγει τα γραφήματα αξιολόγησης (κατανομή σκορ, ROC/PR, feature importance,
  detection ανά τύπο ανωμαλίας) και την ανάλυση ανά ομάδες (department / role /
  sensitivity).

ΠΡΟΑΙΡΕΤΙΚΟ TASK (Ανάλυση δικαιοσύνης / Fairness):
  Υπολογίζει Precision/Recall/F1/FPR/FNR ανά τμήμα και ανά ρόλο, ώστε να
  εντοπιστούν δυσανάλογα ποσοστά ψευδώς θετικών (FPR) ανά ομάδα (πυλώνας
  fairness του Responsible AI). Παράγει και το σχετικό γράφημα.

TASK 7 (Προσομοίωση ολίσθησης & διακυβέρνηση):
  Προσομοιώνει ένα σενάριο "εβδομάδας τηλεργασίας" (concept drift) πάνω στο eval
  window ΧΩΡΙΣ επανεκπαίδευση, μετρά την κατάρρευση του precision, και δοκιμάζει
  ένα mitigation (προσωρινός κανόνας "corporate VPN -30 πόντοι").

Διαβάζει τα ενδιάμεσα artifacts (_eval.pkl / _models.pkl) του 03_ai_pipeline.py.
Έξοδος: figures/fig1..fig6, data/fairness_by_*.csv, data/drift_summary.csv
=============================================================================
"""
...
import pandas as pd
import matplotlib
matplotlib.use('Agg')       # backend χωρίς οθόνη (αποθήκευση σε αρχεία)
...
CLR_AI, CLR_BASE = '#1f6feb', '#d1495b'     # μπλε = AI, κόκκινο = baseline

# Φόρτωση των ενδιάμεσων artifacts που παρήγαγε το 03_ai_pipeline.py.
...
# Ο baseline στο ΙΔΙΟ eval window (για δίκαιη σύγκριση στα γραφήματα).
...
# ---------------------------------------------------------------------------
# [TASK 6] Figure 1: κατανομή του AI risk score (normal vs anomaly)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# [TASK 6] Figure 2: καμπύλες ROC + Precision-Recall, AI vs baseline
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# [TASK 6] Figure 3: σημαντικότητα χαρακτηριστικών (permutation importance)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# [TASK 6] Figure 4: detection ανά τύπο ανωμαλίας, baseline vs AI
#          (αναδεικνύει ότι το AI καλύπτει τα τυφλά σημεία του baseline)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# [TASK 6 / ΠΡΟΑΙΡΕΤΙΚΟ TASK] Ανάλυση ανά ομάδες (department / role / sensitivity)
# ---------------------------------------------------------------------------
def group_metrics(frame, col):
    """Υπολογισμός μετρικών ανά τιμή της στήλης col (για fairness/breakdown)."""
    rows = []
    for g, sub in frame.groupby(col):
        ...
        fpr = fp / n_norm if n_norm else 0     # ψευδώς θετικά (κόστος για νόμιμους)
        fnr = fn / n_anom if n_anom else 0     # διαφυγόντα (κόστος ασφάλειας)
...
# [ΠΡΟΑΙΡΕΤΙΚΟ] Figure 5: FPR ανά τμήμα και ανά ρόλο (fairness analysis)
...
# ---------------------------------------------------------------------------
# [TASK 7] Σενάριο DRIFT: "εβδομάδα τηλεργασίας" πάνω στο eval window
# ---------------------------------------------------------------------------
drift = ev.copy()
rng = np.random.default_rng(25012)
# Το 100% των συνδέσεων μετακινείται από το εταιρικό LAN -> VPN/οικιακά δίκτυα.
...
# Μετατοπισμένα ωράρια -> περισσότερα off-hours. Μερίδιο νέων προσωπικών συσκευών.
...
# Το user_hour_deviation θα ήθελε ιστορικό. Προσεγγίζεται προσθέτοντας το shift.
drift['user_hour_deviation'] = (drift['user_hour_deviation'] + shift).astype(float)
# Επαναϋπολογισμός iso_score και AI score στα drifted features (ΧΩΡΙΣ retraining).
...
def eval_block(frame, proba_col='risk_ai'):
    """Μετρικές με τα ίδια κατώφλια/overrides, για σύγκριση before/after drift."""
...
# [TASK 7] Mitigation: προσωρινός κανόνας "εταιρικό VPN -30 πόντοι" (rule update).
...
# [TASK 7] Figure 6: επίδραση του drift και του mitigation
...
# Αποθήκευση σύνοψης drift (before / after / after_mitigation).
