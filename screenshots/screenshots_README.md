...
- **`02_baseline_policy.py`** — static baseline. Catches the simple types (100%)
  but fails completely (0%) on the context-dependent `Bulk_Data_Spike` and
...
- **`04_drift_fairness_plots.py`** — group breakdown (department / role /
  sensitivity), drift simulation (precision 0.58 → 0.15) and generation of the
  6 figures.
